    ...
    override suspend fun signInWithCredentials(
        ...
        return try {
            when (val result = authManager.signIn(providerType, credential)) {
                ...
                is Result.Failure -> Result.Failure(SocialMediaError.InvalidCredential(""))
                else -> Result.Failure(SocialMediaError.InvalidCredential(""))
            }
        } catch (e: Exception) {
            Logger.e("Exception caught while signing in with credentials: ${e.message}")
            Result.Failure(SocialMediaError.InvalidCredential(""))
        }
    }
    ...
    override suspend fun linkWithCredential(credential: AuthCredential): Result<FirebaseUserInfoDomainModel, SocialMediaError> {
        return try {
            ...
            if (result?.user != null) {
                ...
                Result.Failure(SocialMediaError.LinkSocialMediaError(""))
            }
        } catch (e: Exception) {
            Logger.e("Exception caught while linking credentials: ${e.message}")
            Result.Failure(SocialMediaError.LinkSocialMediaError(e.message ?: ""))
        }
    }
