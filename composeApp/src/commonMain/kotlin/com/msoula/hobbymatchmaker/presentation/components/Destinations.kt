...
class SignInDestination(private val googleUIClient: GoogleUIClient) : Screen {
    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow

        val signInViewModel = koinScreenModel<SignInViewModel> { parametersOf(googleUIClient) }
        val oneTimeEventChannelFlow = signInViewModel.oneTimeEventChannelFlow

        Logger.d("GoogleUIClient in Destinations is: $googleUIClient")
        ...
        SignInScreenContent(
            ...
            redirectToSignUpScreen = {
                navigator.replaceAll(SignUpDestination(googleUIClient))
            }
        )
    }
}

class SignUpDestination(val googleUIClient: GoogleUIClient) : Screen {
    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow
        ...
        SignUpScreenContent(
            redirectToSignInScreen = {
                navigator.replaceAll(SignInDestination(googleUIClient))
            },
            redirectToMovieScreen = {
                navigator.push(MovieDestination())
            },
